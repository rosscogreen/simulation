...
class HighwayEnv(gym.Env):
    ...
    def reset(self):
        ...
        return self.observation.observe(0)
    ...
    def _create_detectors(self):
        for x in [10, 120, 250, 380, 490]:
            self.road.add_detector('upstream', x)
            self.road.add_detector('downstream', x)
    ...
    def _observe(self):
        dt = self.n_intermediate_steps * self.dt
        return self.observation.observe(dt)
        #return self.road.update_detectors_period()
    ...
    def _action_step(self, action=None):
        if action is None:
            return
